...
export class Encrypter {
  ...
  */
  ...
  private createECDH = (key: string): crypto.ECDH => {
    ...
    ecdh.setPrivateKey(
      Buffer.from(this.encrypt("verysuper32bitsecretkeytopsecret", key))
    );
    return ecdh;
  };
   ...
   */
  public encryptMessage = (text: string, receiver: string): string => {
    let otherPublicKey = this.getOtherPublicKey(receiver);
    let sharedKey = this.getSharedKey(otherPublicKey);
   ...
   */
  public decryptMessage = (text: string, sender: string): string => {
    let otherPublicKey = this.getOtherPublicKey(sender);
    let sharedKey = this.getSharedKey(otherPublicKey);
  ...
  private getOtherPublicKey = (receiver: string): string => {
    let otherEcdh = this.createECDH(receiver);
    let otherPublicKey = otherEcdh.getPublicKey().toString("hex");
...
//encryptMessage metodu kullanılarak "Hello World" metni "5553332211" telefon numarasına şifreleniyor ve şifreli metin "encryptedMessage" değişkenine atanıyor.
let encryptedMessage = encrypterSender.encryptMessage(
  "Hello World",
  "5553332211"
);
...
//decryptMessage metodu kullanılarak "encryptedMessage" değişkenindeki şifreli metin "5554443322" şifreleme anahtarı kullanılarak çözülüyor ve orijinal metin "decryptedMessage" değişkenine atanıyor.
let decryptedMessage = encrypterReceiver.decryptMessage(
  encryptedMessage,
  "5554443322"
);
